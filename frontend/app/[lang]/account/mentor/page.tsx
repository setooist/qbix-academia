...
export default function MentorDashboard() {
    ...
    useEffect(() => {
        async function fetchData() {
            if (user) {
                try {
                    // In production, filter by mentor ID
                    // For now, fetch all and we'll filter client-side or show all for mentors
    ...
    const pendingReview = activities.filter((a) =>
        a.activityStatus === 'submitted' || a.activityStatus === 'under_review'
    );
    const inProgress = activities.filter((a) =>
        a.activityStatus === 'in_progress' || a.activityStatus === 'changes_requested'
    );
    const approved = activities.filter((a) => a.activityStatus === 'approved');
    const assigned = activities.filter((a) => a.activityStatus === 'assigned');
    ...
    const stats = [
        ...
        { label: 'Total Assigned', value: activities.length, icon: Users, color: 'text-purple-600 bg-purple-100' },
    ];
                                        ...
                                        <Table>
                                            ...
                                            <TableBody>
                                                {pendingReview.map((activity) => (
                                                    <TableRow key={activity.documentId} className="hover:bg-yellow-50">
                                                        <TableCell className="font-medium">{activity.title}</TableCell>
                                                        <TableCell>{activity.assignee?.username || 'Unknown'}</TableCell>
                                                        <TableCell>
                                                            {activity.dueDate
                                                        ...
                                                        <TableCell className="text-right">
                                                            <Button
                                                                size="sm"
                                                                onClick={() => router.push(`/account/mentor/${activity.documentId}`)}
                                                            >
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <ActivityTable
                                    ...
                                    onView={(id) => router.push(`/account/mentor/${id}`)}
                                />
                            </CardContent>
                        </Card>
                    </TabsContent>
            ...
            <Table>
                ...
                <TableBody>
                    {activities.map((activity) => (
                        <TableRow key={activity.documentId}>
                            <TableCell className="font-medium">{activity.title}</TableCell>
                            <TableCell>{activity.assignee?.username || 'Unknown'}</TableCell>
                            <TableCell>
                                {activity.dueDate
                            ...
                            {showGrade && (
                                <TableCell>
                                    <span className="font-bold">{activity.grade ?? '-'}</span>
                                    <span className="text-gray-400">/100</span>
                                </TableCell>
                            )}
                            <TableCell className="text-right">
                                <Button variant="outline" size="sm" onClick={() => onView(activity.documentId)}>
                                    View
